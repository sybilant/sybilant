...
** Roadmap
   ...
   - [-] version 0.7
     ...
     - [-] analyze defasm form
       ...
       - [X] check tag for instructions that change register types (%bsf, %bsr,
         %cbw, %cdqe, %cwde, %mov, %movsx, %movsxd, %movzx, %xchg)
       - [X] unsigned extend tag when a 32-bit register is used as a 64-bit
         register
       - [ ] check tag for instructions with weird types (%cdq, %cqo, %cwd,
         %div, %idiv, %imul, %mul, %rdtsc)
   - [ ] version 0.8
     - [ ] parse tuple types
